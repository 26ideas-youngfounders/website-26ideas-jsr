...
CREATE TABLE public.country_codes (
  ...
  country_code VARCHAR(10) NOT NULL UNIQUE, -- e.g., "+91"
...
-- Insert common country codes with flag emojis, prioritizing India
INSERT INTO public.country_codes (country_name, country_code, iso_code, country_flag_emoji) VALUES
('India', '+91', 'IN', '🇮🇳'),
('United States', '+1', 'US', '🇺🇸'),
...
-- Add country_code column to individuals table
ALTER TABLE public.individuals 
ADD COLUMN country_code VARCHAR(10) DEFAULT '+91' REFERENCES public.country_codes(country_code);

-- Update existing mobile column comment for clarity
...
-- Create index for better performance on country code lookups
CREATE INDEX idx_country_codes_iso_code ON public.country_codes(iso_code);
CREATE INDEX idx_country_codes_country_code ON public.country_codes(country_code);
CREATE INDEX idx_individuals_country_code ON public.individuals(country_code);
...
-- Update mentor_applications table to include country_code
ALTER TABLE public.mentor_applications 
ADD COLUMN country_code VARCHAR(10) DEFAULT '+91' REFERENCES public.country_codes(country_code);

-- Update the create_or_update_mentor_profile function to handle country code
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.country_codes WHERE country_code = p_country_code AND is_active = true) THEN
  ...
  SELECT individual_id, email, mobile, country_code, is_mentor, mentor_status
  INTO v_existing_record
  FROM public.individuals 
  WHERE email = p_email OR (mobile = p_mobile AND country_code = p_country_code)
  LIMIT 1;
  ...
  ELSE
    ...
    INSERT INTO public.individuals (
      first_name, last_name, email, mobile, country_code, city, country,
      ...
      p_first_name, p_last_name, p_email, p_mobile, p_country_code, p_city, p_country,
  ...
  INSERT INTO public.mentor_applications (
    ...
    availability_time, availability_notes, linkedin_url, instagram_handle, country_code
  ) VALUES (
    v_individual_id, p_topics_of_interest, p_availability_days,
    p_availability_time, p_availability_notes, p_linkedin, p_instagram, p_country_code
